...
const AdminPage = () => {
    ...
    const fetchPending = useCallback(async () => {
        try {
            ...
            const data = r.data.pending_users || r.data.users || r.data
    ...
    const fetchUsers = useCallback(async () => {
        try {
            const r = await apiClient.get('/api/admin/users', authHeader)
    ...
    const fetchDeposits = useCallback(async () => {
        try {
            ...
            const data = r.data.pending_deposits || r.data.deposits || r.data
    ...
    const fetchAllDeposits = useCallback(async () => {
        try {
            ...
            const data = r.data.deposits
            setAllDeposits(Array.isArray(data) ? data : [])
            setDepositSummary(r.data.summary || null)
        } catch { setAllDeposits([]) }
    }, [adminToken])

    const fetchResets = useCallback(async () => {
        try {
            const r = await apiClient.get('/api/admin/requests/password-resets', authHeader)
    ...
    const fetchNotifications = useCallback(async () => {
        try {
            const r = await apiClient.get('/api/admin/notifications', authHeader)
                                ...
                                <Card className="glass border-purple-500/20 bg-purple-500/5">
                                    <CardContent className="p-4 space-y-3">
                                        <div className="flex justify-between items-center">
                                            ...
                                            <span className="text-xs font-bold">{stats.system_health.memory?.percent || 0}%</span>
                                        </div>
                                        <div className="h-2 w-full bg-white/10 rounded-full overflow-hidden">
                                            <div className="h-full bg-purple-500 transition-all duration-500" style={{ width: `${stats.system_health.memory?.percent || 0}%` }} />
                                        </div>
                                        <p className="text-[10px] text-muted-foreground">{(stats.system_health.memory?.used_gb || 0).toFixed(1)}GB / {(stats.system_health.memory?.total_gb || 0).toFixed(1)}GB used</p>
                                    </CardContent>
                                </Card>
                                {/* Disk */}
                                <Card className="glass border-orange-500/20 bg-orange-500/5">
                                    <CardContent className="p-4 space-y-3">
                                        <div className="flex justify-between items-center">
                                            ...
                                            <span className="text-xs font-bold">{stats.system_health.disk?.percent || 0}%</span>
                                        </div>
                                        <div className="h-2 w-full bg-white/10 rounded-full overflow-hidden">
                                            <div className="h-full bg-orange-500 transition-all duration-500" style={{ width: `${stats.system_health.disk?.percent || 0}%` }} />
                                        </div>
                                        <p className="text-[10px] text-muted-foreground">{(stats.system_health.disk?.used_gb || 0).toFixed(0)}GB / {(stats.system_health.disk?.total_gb || 0).toFixed(0)}GB</p>
                                    </CardContent>
                                </Card>
                            </div>
                            <div className="flex flex-wrap gap-4 text-xs text-muted-foreground px-1">
                                <div className="flex items-center gap-1.5"><Wifi className="w-3 h-3" /> Net Sent: {(stats.system_health.network?.sent_mb || 0).toFixed(2)} MB</div>
                                <div className="flex items-center gap-1.5"><Wifi className="w-3 h-3" /> Net Recv: {(stats.system_health.network?.received_mb || 0).toFixed(2)} MB</div>
                                <div className="flex items-center gap-1.5"><Clock className="w-3 h-3" /> Uptime: {((stats.system_health.uptime_seconds || 0) / 3600).toFixed(1)} hours</div>
                            </div>
                        </div>
                    )}
