...
import {
    ...
    AlertTriangle, TrendingUp, Wallet, IndianRupee, Zap, Rocket
} from 'lucide-react'
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div className="flex gap-2">
                    <Button variant="outline" onClick={() => navigate('home')}>
                        <Rocket className="w-4 h-4 mr-2" /> Home
                    </Button>
            ...
            <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
                {[
                    { label: 'Wallet Balance', val: `₹${user.credits?.toFixed(2) ?? '0.00'}`, icon: Wallet, color: 'text-blue-600 dark:text-blue-400', bg: 'bg-blue-500/15' },
                    { label: 'Total Searches', val: user.searches ?? 0, icon: BarChart3, color: 'text-purple-600 dark:text-purple-400', bg: 'bg-purple-500/15' },
                    { label: 'Credits Spent', val: `₹${creditUsed.toFixed(2)}`, icon: TrendingUp, color: 'text-orange-600 dark:text-orange-400', bg: 'bg-orange-500/15' },
                    { label: 'Account', val: user.account_status || 'ACTIVE', icon: CheckCircle2, color: 'text-green-600 dark:text-green-400', bg: 'bg-green-500/15' },
                ].map(s => (
            ...
            {/* Charge Notice */}
            <div className="flex items-start gap-3 p-4 rounded-xl glass border-yellow-500/30 border bg-yellow-500/10 text-sm">
                <AlertTriangle className="w-5 h-5 text-yellow-600 dark:text-yellow-500 shrink-0 mt-0.5" />
                <div>
                    <p className="font-medium text-yellow-700 dark:text-yellow-400">Billing Notice</p>
            ...
            <Card className="glass border-primary/20">
                ...
                <CardContent className="space-y-4">
                    ...
                    <div className="glass rounded-xl p-4 space-y-3 border border-white/5">
                        ...
                        <Button variant="outline" size="sm" className="w-full mt-2" onClick={() => navigate('api-docs')}>
                            <Code2 className="w-4 h-4 mr-2" /> View Full API Documentation
                        </Button>
                    </div>
